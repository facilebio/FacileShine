...
#'
#' @export
#' @rdname reactiveFacileDataStore_v1
#'
#' @importFrom shiny selectInput renderUI uiOutput
#' @param dataset The FacileDataStore to make reactive
#' @param restrict_samples A predefined set of samples to subset the dataset to.
#' @param user the custom_key to use for ephemeral sample and feature
#'   annotations that are saved back to the `dataset`.
reactiveFacileDataStore_v1 <- function(input, output, session, dataset,
                                       restrict_samples = NULL,
                                       user = Sys.getenv("USER"), ...) {
  assert_class(dataset, "FacileDataStore")

  if (!is.null(restrict_samples)) {
    restrict_samples <- collect(assert_sample_subset(restrict_samples), n = Inf)
    restrict_samples <- as_facile_frame(restrict_samples, dataset)
  } else {
    restrict_samples <- collect(samples(dataset), n = Inf)
  }

  state <- reactiveValues(
    active_samples = restrict_samples)
  ...
  if (!isS4(dataset)) {
    ...
    class(dataset) <- c("ReactiveFacileDataStore_v1", "ReactiveFacileDataStore",
                        class(dataset))
  } else {
...
reactiveFacileDataStore_v1UI <- function(id, ...) {
  ns <- NS(id)
...
#' @noRd
#' @export
reactives.ReactiveFacileDataStore_v1 <- function(x, name = NULL, ...) {
  if (isS4(x)) {
...
#' @noRd
#' @export
user.ReactiveFacileDataStore_v1 <- function(x, ...) {
  reactives(x, "user")
}

#' @rdname reactiveFacileDataStore
#' @export
active_assays.ReactiveFacileDataStore_v1 <- function(x, ...) {
  assert_reacting()
...
#' @noRd
#' @export
active_covariates.ReactiveFacileDataStore_v1 <- function(x, ...) {
  assert_reacting()
...
#' @export
#' @rdname reactiveFacileDataStore
active_samples.ReactiveFacileDataStore_v1 <- function(x, ...) {
  assert_reacting()
...
update_reactive_samples.ReactiveFacileDataStore_v1 <- function(x,
                                                            active_samples,
                                                            criterion = NULL,
                                                            ...) {
  assert_reacting()
  restrict_samples <- reactives(x, ".restrict_samples")
  ...
  if (!is.same) {
    .state <- reactives(x, ".state")
    .state$active_samples <- .as
  }
...
#'
#' Updates the ephemeral covariates during an analysis.
#'
...
#'   collect() %>%
#'   sample_n(20) %>%
#'   mutate(variable = "test_anno",
#'          value = sample(c("a", "b"), 20, replace = TRUE),
#'          class = "categorical",
#'          type = "ephemeral",
#'          date_entered = as.integer(Sys.time()))
#' ufds <- update_reactive_covariates(fds, covs, "some_module")
update_reactive_covariates.ReactiveFacileDataStore_v1 <- function(rfds,
                                                               covariates,
                                                               namespace, ...) {
...
name.ReactiveFacileDataStore_v1 <- function(x, ...) {
  FacileData:::name.FacileDataSet(x)
}
...
#' @noRd
#' @export
fetch_sample_covariates.ReactiveFacileDataStore_v1 <- function(
  ...
  stored.covs <- x %>%
    FacileData:::fetch_sample_covariates.FacileDataSet(samples = samples,
