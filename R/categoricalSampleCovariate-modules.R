...
# Original Covariate Selector ==================================================

#' A selectInput that provides the categorical covariates over a FacileDataStore
#'
#' This was the original implementation of this module. It did not accept an
#' optional (sub)set of samples to get covariates from.
#'
...
categoricalSampleCovariateSelect.orig <- function(input, output, session, rfds,
                                                  universe = NULL, ...,
                                                  .with_none = TRUE,
                                                  .exclude = NULL,
                                                  .reactive = TRUE) {
  assert_multi_class(rfds, c("ReactiveFacileDataStore", "reactive_facile_frame"))
  ...
  active.covariates <- reactive({
    ...
    # updates when ephemeral covariates fire, even if we don't want to respond
    # when covariates change due to cohort changes.
    depend(rfds, "covariates")
